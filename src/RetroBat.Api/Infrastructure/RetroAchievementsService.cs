...
public sealed class RetroAchievementsService : IDisposable, IProvider
{
    private const int MaxLeaderboardEntries = 50000;
    private const int LeaderboardEntriesPageSize = 500;
    private const int WarmLeaderboardEntriesCount = 5000;
    private const int MaxWarmLeaderboardBoards = 64;
    ...
    private readonly IngameGameplayStateService _gameplayState;
    private readonly HttpClient _proxyClient = new();
    private readonly HttpClient _apiClient = new();
    private readonly SemaphoreSlim _sessionLock = new(1, 1);
    private readonly ConcurrentDictionary<string, SemaphoreSlim> _fileLocks = new(StringComparer.OrdinalIgnoreCase);
    private readonly ConcurrentDictionary<int, RecentLeaderboardSubmission> _recentLeaderboardSubmissions = new();
    private readonly ConcurrentDictionary<int, byte> _leaderboardEntryWarmups = new();
    ...
    public RetroAchievementsService(
        ...
        IngameGameplayStateService gameplayState)
    {
        _options = options;
        _eventBus = eventBus;
    ...
    {
        _subscription = _eventBus.Subscribe<EventEnvelope>(HandleRuntimeCoherenceEvent);
    ...
    public async Task EndSessionAsync(string reason, CancellationToken cancellationToken = default)
    {
        if (_session.GameId == null && string.IsNullOrWhiteSpace(_session.GameTitle)) return;
        var ended = _session.Clone();
        await PublishAsync("retroachievements.session.ended", new
        {
            Source = "retroachievements.runtime",
            Reason = string.IsNullOrWhiteSpace(reason) ? "unknown" : reason,
            Session = ended
        }, cancellationToken);

        _recentLeaderboardSubmissions.Clear();

        await UpdateSessionAsync(snapshot =>
        {
            snapshot.GameId = null;
            snapshot.GameTitle = string.Empty;
            snapshot.GameHash = string.Empty;
            snapshot.GameImageIconUrl = string.Empty;
            snapshot.GameImageIconRemoteUrl = string.Empty;
            snapshot.Score = null;
            snapshot.SoftcoreScore = null;
            snapshot.RichPresence = string.Empty;
            snapshot.AchievementCount = null;
            snapshot.AchievementsById.Clear();
            snapshot.LeaderboardCount = null;
            snapshot.LeaderboardsById.Clear();
            snapshot.CatalogRevision = string.Empty;
            snapshot.SessionUnlockedAchievements.Clear();
            snapshot.LastEventAt = DateTime.UtcNow;
        }, cancellationToken);
    }

    public Task PublishChallengeAsync(RetroAchievementsChallengeEvent value, string? correlationId = null, CancellationToken cancellationToken = default)
        => PublishAsync("retroachievements.challenge.changed", value, cancellationToken, correlationId);

    public Task PublishLeaderboardAsync(RetroAchievementsLeaderboardEvent value, string? correlationId = null, CancellationToken cancellationToken = default)
    {
        NormalizeLeaderboardEvent(value);
        return PublishAsync("retroachievements.leaderboard.changed", value, cancellationToken, correlationId);
    }

    private void NormalizeLeaderboardEvent(RetroAchievementsLeaderboardEvent value)
    {
        if (value.LeaderboardId is not > 0) return;

        value.GameId ??= _session.GameId;
        if (string.IsNullOrWhiteSpace(value.User)) value.User = _session.Username;
        if (_session.LeaderboardsById.TryGetValue(value.LeaderboardId.Value, out var leaderboard))
        {
        ...
        if (!IsLeaderboardResultState(value.State) ||
            !_recentLeaderboardSubmissions.TryGetValue(value.LeaderboardId.Value, out var submission) ||
            DateTime.UtcNow - submission.ObservedAtUtc > TimeSpan.FromMinutes(2))
        {
            return;
        }

        value.Value = submission.Value;
        value.FormattedValue = submission.FormattedValue;
        value.Rank = submission.Rank;
        value.PersonalBestScore = submission.BestScore;
        value.PersonalBestFormattedScore = submission.BestFormattedScore;
        if (string.IsNullOrWhiteSpace(value.Title)) value.Title = submission.Title;
        if (string.IsNullOrWhiteSpace(value.User)) value.User = submission.User;
        if (submission.ReferenceEntries.Count > 0)
        {
            value.ReferenceEntries = submission.ReferenceEntries.ToList();
            var reference = submission.ReferenceEntries[0];
    ...
    public bool AreLeaderboardsEnabled()
    {
        ...
        return bool.TryParse(TryGetSetting(settings, "global.retroachievements.leaderboards"), out var enabled) && enabled;
    }

    public async Task<IReadOnlyList<RetroAchievementsLeaderboardEntry>> GetLeaderboardEntriesAsync(
        int leaderboardId,
        int count = 25,
        CancellationToken cancellationToken = default)
    {
        if (leaderboardId <= 0 || !AreLeaderboardsEnabled()) return Array.Empty<RetroAchievementsLeaderboardEntry>();
        var targetCount = Math.Clamp(count, 1, MaxLeaderboardEntries);
        var result = new List<RetroAchievementsLeaderboardEntry>(Math.Min(targetCount, LeaderboardEntriesPageSize));
        for (var offset = 0; offset < targetCount; offset += LeaderboardEntriesPageSize)
        {
            var pageSize = Math.Min(LeaderboardEntriesPageSize, targetCount - offset);
            var document = await GetApiJsonAsync(
                ...
                $"API_GetLeaderboardEntries_leaderboard_{leaderboardId}_top_{targetCount}_offset_{offset}.json",
                preferFresh: false,
                cancellationToken);
            if (document == null) break;
            using (document)
            {
                var page = ExtractLeaderboardEntries(document.RootElement);
                if (page.Count == 0) break;
                result.AddRange(page);
                if (page.Count < pageSize || result.Count >= targetCount) break;
            }
        }
        return result.Count > targetCount ? result.Take(targetCount).ToList() : result;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var richPresence = action.Equals("ping", StringComparison.OrdinalIgnoreCase)
            ...
            : string.Empty;
        var acceptedRichPresence = ResolveAcceptedRichPresence(richPresence);

        if (string.IsNullOrWhiteSpace(username) &&
            gameId == null &&
            string.IsNullOrWhiteSpace(gameHash) &&
            hardcore == null &&
            string.IsNullOrWhiteSpace(acceptedRichPresence))
        {
            return;
        }
        ...
        await UpdateSessionAsync(snapshot =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(acceptedRichPresence))
            {
                snapshot.RichPresence = acceptedRichPresence;
            }
        ...
        try
        {
            ...
            switch (action.ToLowerInvariant())
            {
                ...
                case "awardachievement":
                    ...
                    break;
                case "submitlbentry":
                    var leaderboardId = TryReadLeaderboardId(parameters);
                    var submittedValue = GetParameter(parameters, "s");
                    var submittedFormattedValue = FormatLeaderboardSubmittedValue(leaderboardId, submittedValue);
                    var submission = ParseLeaderboardSubmission(parsed?.RootElement, leaderboardId, submittedValue, submittedFormattedValue);
                    if (ok && leaderboardId is > 0)
                    {
                        _recentLeaderboardSubmissions[leaderboardId.Value] = submission;
                    }
                    var leaderboardResponse = SummarizeJson(parsed);
                    await PublishAsync("retroachievements.leaderboard.submit.confirmed", new
                    {
                        ...
                        FormattedValue = submission.FormattedValue,
                        submission.Rank,
                        submission.NumEntries,
                        submission.BestScore,
                        submission.BestFormattedScore,
                        ReferenceEntries = submission.ReferenceEntries,
                        Response = JsonSerializer.Serialize(leaderboardResponse, JsonOptions)
                    }, cancellationToken, correlationId);
                    await PublishLeaderboardAsync(new RetroAchievementsLeaderboardEvent
                    {
                        Source = "proxy",
                        State = ok ? "submitted" : "failed",
                        GameId = _session.GameId,
                        LeaderboardId = leaderboardId,
                        ...
                        PersonalBestScore = submission.BestScore,
                        PersonalBestFormattedScore = submission.BestFormattedScore,
                        ReferenceRank = submission.ReferenceEntries.FirstOrDefault()?.Rank,
                        ReferenceUser = submission.ReferenceEntries.FirstOrDefault()?.User ?? string.Empty,
                        ReferenceFormattedScore = submission.ReferenceEntries.FirstOrDefault()?.FormattedScore ?? string.Empty,
                        ReferenceEntries = submission.ReferenceEntries.ToList(),
                        Response = JsonSerializer.Serialize(leaderboardResponse, JsonOptions)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var leaderboardInfos = ExtractLeaderboardInfos(response?.RootElement);
        ...
        HydrateLeaderboardMediaLinks(leaderboardInfos.Values);
        ...
        await UpdateSessionAsync(snapshot =>
        {
            ...
            {
                snapshot.LeaderboardsById = leaderboardInfos;
                snapshot.LeaderboardCount = leaderboardInfos.Count;
            }
            snapshot.CatalogRevision = BuildCatalogRevision(snapshot.AchievementsById, snapshot.LeaderboardsById);
        ...
        QueueLeaderboardBadgeDownloads(leaderboardInfos.Values);
        var imageIconCachePath = ToCachedMediaPath(imageIconUrl, "game_images");
        await PublishCatalogAsync(cancellationToken, correlationId);
        ...
        await PublishAsync("retroachievements.achievementsets.loaded", new
        {
            ...
            LeaderboardCount = leaderboardInfos.Count,
            CatalogRevision = _session.CatalogRevision,
        ...
        CancellationToken cancellationToken)
    {
        ...
        await PublishAsync("retroachievements.richpresence.ping", new
        {
            ...
            GameTitle = _session.GameTitle,
            GameHash = gameHash,
            RichPresence = acceptedRichPresence,
            RawRichPresence = richPresence,
            RichPresenceSuppressed = !string.IsNullOrWhiteSpace(richPresence) &&
                string.IsNullOrWhiteSpace(acceptedRichPresence),
            Gameplay = _gameplayState.GetSnapshot(),
            Response = SummarizeJson(response)
        }, cancellationToken, correlationId);
    }
    ...
    private string ResolveAcceptedRichPresence(string richPresence)
    {
        ...
        var trimmed = richPresence.Trim();
        return _gameplayState.ShouldSuppressRichPresence(trimmed)
            ? string.Empty
            : trimmed;
    }

    private void HandleRuntimeCoherenceEvent(EventEnvelope envelope)
    {
        ...
        var currentRichPresence = _session.RichPresence;
        if (string.IsNullOrWhiteSpace(currentRichPresence) ||
            !_gameplayState.ShouldSuppressRichPresence(currentRichPresence))
        {
            return;
        }

        _ = UpdateSessionAsync(snapshot =>
        {
            if (_gameplayState.ShouldSuppressRichPresence(snapshot.RichPresence))
            {
                snapshot.RichPresence = string.Empty;
                snapshot.LastEventAt = DateTime.UtcNow;
            }
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(username))
                {
                    ...
                    await WarmUserGameLeaderboardsAsync(gameId.Value, username, CancellationToken.None);
                }
                if (AreLeaderboardsEnabled())
                {
                    await WarmLeaderboardCatalogAsync(gameId.Value, CancellationToken.None);
                }
            }
            catch
            {
    ...
    private async Task WarmLeaderboardCatalogAsync(int gameId, CancellationToken cancellationToken)
    {
        var document = await GetApiJsonAsync(
            "API_GetGameLeaderboards.php",
            new Dictionary<string, string> { ["i"] = gameId.ToString(), ["c"] = "500", ["o"] = "0" },
            $"API_GetGameLeaderboards_game_{gameId}.json",
            preferFresh: false,
            cancellationToken);
        if (document == null) return;

        var enriched = new Dictionary<int, RetroAchievementsLeaderboardInfo>();
        using (document)
        {
            if (!TryGetProperty(document.RootElement, "Results", out var results) || results.ValueKind != JsonValueKind.Array) return;
            foreach (var item in results.EnumerateArray())
            {
                var id = ReadInt(item, "ID") ?? ReadInt(item, "Id");
                if (id is not > 0) continue;
                var info = new RetroAchievementsLeaderboardInfo
                {
                    Id = id.Value,
                    Title = ReadString(item, "Title") ?? string.Empty,
                    Description = ReadString(item, "Description") ?? string.Empty,
                    Format = ReadString(item, "Format") ?? string.Empty,
                    LowerIsBetter = ReadBool(item, "RankAsc") ?? false,
                    BadgeName = ReadString(item, "BadgeName") ?? ReadString(item, "Badge") ?? string.Empty
                };
                HydrateLeaderboardMediaLinks(new[] { info });
                if (TryGetProperty(item, "TopEntry", out var topEntry) && topEntry.ValueKind == JsonValueKind.Object)
                {
                    info.TopUser = ReadString(topEntry, "User") ?? string.Empty;
                    info.TopScore = ReadLong(topEntry, "Score");
                    info.TopFormattedScore = ReadString(topEntry, "FormattedScore") ?? string.Empty;
                }
                enriched[info.Id] = info;
            }
        }
        ...
        try
        {
            if (_session.GameId != gameId) return;
            var copy = _session.Clone();
            foreach (var pair in enriched)
            {
                if (copy.LeaderboardsById.TryGetValue(pair.Key, out var current))
                {
                    current.Title = FirstNonEmpty(current.Title, pair.Value.Title);
                    current.Description = FirstNonEmpty(current.Description, pair.Value.Description);
                    current.Format = FirstNonEmpty(current.Format, pair.Value.Format);
                    current.LowerIsBetter = pair.Value.LowerIsBetter;
                    current.TopUser = pair.Value.TopUser;
                    current.TopScore = pair.Value.TopScore;
                    current.TopFormattedScore = pair.Value.TopFormattedScore;
                    current.BadgeName = FirstNonEmpty(current.BadgeName, pair.Value.BadgeName);
                    current.BadgeUrl = FirstNonEmpty(current.BadgeUrl, pair.Value.BadgeUrl);
                    current.BadgeRemoteUrl = FirstNonEmpty(current.BadgeRemoteUrl, pair.Value.BadgeRemoteUrl);
                }
                else copy.LeaderboardsById[pair.Key] = pair.Value;
            }
            copy.LeaderboardCount = copy.LeaderboardsById.Count;
            copy.CatalogRevision = BuildCatalogRevision(copy.AchievementsById, copy.LeaderboardsById);
            _session = copy;
        }
        finally
        {
        ...
        await PublishCatalogAsync(cancellationToken);
        _ = WarmLeaderboardEntriesAsync(enriched.Values.ToList(), CancellationToken.None);
    }

    private async Task WarmLeaderboardEntriesAsync(IReadOnlyCollection<RetroAchievementsLeaderboardInfo> leaderboards, CancellationToken cancellationToken)
    {
        if (leaderboards.Count == 0 || !AreLeaderboardsEnabled())
        {
            return;
        }

        var targets = leaderboards
            .Where(item => item.Id > 0 && !item.Hidden)
            .OrderByDescending(IsTimedLeaderboard)
            .ThenBy(item => item.Id)
            .Take(MaxWarmLeaderboardBoards)
            .ToArray();

        foreach (var leaderboard in targets)
        {
            if (!_leaderboardEntryWarmups.TryAdd(leaderboard.Id, 0))
            {
                continue;
            }
            ...
            try
            {
                await GetLeaderboardEntriesAsync(leaderboard.Id, WarmLeaderboardEntriesCount, cancellationToken);
            }
            catch
            {
                // Background cache warmup must never slow or break gameplay.
            }
            finally
            {
                _leaderboardEntryWarmups.TryRemove(leaderboard.Id, out _);
            }
        }
    }
    ...
    private async Task WarmUserGameLeaderboardsAsync(int gameId, string username, CancellationToken cancellationToken)
    {
        if (gameId <= 0 || string.IsNullOrWhiteSpace(username) || !AreLeaderboardsEnabled())
        {
            return;
        }

        const int pageSize = 500;
        for (var offset = 0; offset < WarmLeaderboardEntriesCount; offset += pageSize)
        {
            var document = await GetApiJsonAsync(
                ...
                $"API_GetUserGameLeaderboards_game_{gameId}_user_{SanitizeFilePart(username)}_top_{WarmLeaderboardEntriesCount}_offset_{offset}.json",
                preferFresh: false,
                cancellationToken);
            if (document == null) break;
            using (document)
            {
                if (!TryGetProperty(document.RootElement, "Results", out var results) ||
                    ...
                    results.GetArrayLength() < pageSize)
                {
                    break;
                }
            }
    ...
    private void QueueLeaderboardBadgeDownloads(IEnumerable<RetroAchievementsLeaderboardInfo> leaderboards)
    {
        if (!_options.CurrentValue.RetroAchievements.Api.DownloadBadges)
        {
            return;
        ...
        var badgeNames = leaderboards
            .Select(leaderboard => NormalizeBadgeFileName(leaderboard.BadgeName))
            .Where(name => !string.IsNullOrWhiteSpace(name))
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .ToArray();
        if (badgeNames.Length == 0)
    ...
    private void HydrateLeaderboardMediaLinks(IEnumerable<RetroAchievementsLeaderboardInfo> leaderboards)
    {
        foreach (var leaderboard in leaderboards)
        {
            var fileName = NormalizeBadgeFileName(leaderboard.BadgeName);
            if (string.IsNullOrWhiteSpace(fileName))
            {
                leaderboard.BadgeUrl = string.Empty;
                leaderboard.BadgeRemoteUrl = string.Empty;
                continue;
            }

            leaderboard.BadgeUrl = Path.Combine(ResolveCacheRoot(), "badges", fileName);
            leaderboard.BadgeRemoteUrl = "https://media.retroachievements.org/Badge/" + Uri.EscapeDataString(fileName);
        }
    }
    ...
    private static Dictionary<int, RetroAchievementsLeaderboardInfo> ExtractLeaderboardInfos(JsonElement? root)
    {
        var leaderboards = new Dictionary<int, RetroAchievementsLeaderboardInfo>();
        if (root != null) CollectLeaderboardInfos(root.Value, leaderboards, insideLeaderboardCollection: false);
        return leaderboards;
    }

    private static void CollectLeaderboardInfos(
        JsonElement element,
        Dictionary<int, RetroAchievementsLeaderboardInfo> leaderboards,
        bool insideLeaderboardCollection)
    {
        switch (element.ValueKind)
        {
            case JsonValueKind.Object:
                if (insideLeaderboardCollection && TryReadLeaderboardInfo(element, out var info))
                    leaderboards[info.Id] = info;
                foreach (var property in element.EnumerateObject())
                {
                    if (property.Value.ValueKind is not (JsonValueKind.Object or JsonValueKind.Array)) continue;
                    var isLeaderboardCollection = insideLeaderboardCollection ||
                                                  property.Name.Equals("Leaderboards", StringComparison.OrdinalIgnoreCase) ||
                                                  property.Name.Equals("Leaderboard", StringComparison.OrdinalIgnoreCase);
                    CollectLeaderboardInfos(property.Value, leaderboards, isLeaderboardCollection);
                }
                break;
            case JsonValueKind.Array:
                foreach (var item in element.EnumerateArray())
                    CollectLeaderboardInfos(item, leaderboards, insideLeaderboardCollection);
                break;
        }
    }

    private static bool TryReadLeaderboardInfo(JsonElement element, out RetroAchievementsLeaderboardInfo info)
    {
        var id = ReadInt(element, "LeaderboardID") ?? ReadInt(element, "LeaderboardId") ??
                 ReadInt(element, "ID") ?? ReadInt(element, "Id");
        var definition = ReadString(element, "Mem") ?? ReadString(element, "Definition") ?? string.Empty;
        var format = ReadString(element, "Format") ?? ReadString(element, "LBFormat") ?? string.Empty;
        info = new RetroAchievementsLeaderboardInfo
        {
            Id = id ?? 0,
            Title = ReadString(element, "Title") ?? ReadString(element, "LBTitle") ?? string.Empty,
            Description = ReadString(element, "Description") ?? ReadString(element, "LBDesc") ?? string.Empty,
            Format = format,
            LowerIsBetter = ReadBool(element, "LowerIsBetter") ?? false,
            Hidden = ReadBool(element, "Hidden") ?? false,
            Definition = definition,
            BadgeName = ReadString(element, "BadgeName") ?? ReadString(element, "Badge") ?? string.Empty
        };
        return id is > 0 && (!string.IsNullOrWhiteSpace(definition) || !string.IsNullOrWhiteSpace(format));
    }

    private static IReadOnlyList<RetroAchievementsLeaderboardEntry> ExtractLeaderboardEntries(JsonElement root)
    {
        if (!TryGetProperty(root, "Results", out var results) || results.ValueKind != JsonValueKind.Array)
            return Array.Empty<RetroAchievementsLeaderboardEntry>();
        var entries = new List<RetroAchievementsLeaderboardEntry>();
        foreach (var item in results.EnumerateArray())
        {
            var rank = ReadInt(item, "Rank");
            var user = ReadString(item, "User") ?? string.Empty;
            if (rank == null || string.IsNullOrWhiteSpace(user)) continue;
            entries.Add(new RetroAchievementsLeaderboardEntry
            {
                Rank = rank.Value,
                User = user,
                Score = ReadLong(item, "Score"),
                FormattedScore = ReadString(item, "FormattedScore") ?? string.Empty
            });
        }
        return entries;
    }
    ...
    private static bool TryReadAchievementInfo(JsonElement element, out RetroAchievementsAchievementInfo info)
    {
        var id = ReadInt(element, "AchievementID") ??
                 ...
                 ReadInt(element, "ID") ??
                 ReadInt(element, "Id");
        var achievementMarker = HasProperty(element, "AchievementID") || HasProperty(element, "AchievementId") ||
                                HasProperty(element, "MemAddr") || HasProperty(element, "Points") ||
                                HasProperty(element, "BadgeName") || HasProperty(element, "BadgeURL");
        ...
        return id != null && achievementMarker && !IsRetroAchievementsWarningInfo(info);
    }
    ...
    private string FormatLeaderboardSubmittedValue(int? leaderboardId, string rawValue)
    {
        if (leaderboardId is not > 0 ||
            !_session.LeaderboardsById.TryGetValue(leaderboardId.Value, out var leaderboard) ||
            !leaderboard.Format.Contains("TIME", StringComparison.OrdinalIgnoreCase) ||
            !long.TryParse(rawValue, NumberStyles.Integer, CultureInfo.InvariantCulture, out var frames))
        {
            return rawValue;
        }

        var totalSeconds = Math.Max(0, frames) / 60d;
        var roundedCentiseconds = (long)Math.Round(totalSeconds * 100d, MidpointRounding.AwayFromZero);
    ...
    private RecentLeaderboardSubmission ParseLeaderboardSubmission(
        JsonElement? root,
        int? leaderboardId,
        string submittedValue,
        string submittedFormattedValue)
    {
        var response = root;
        if (root != null && TryGetProperty(root.Value, "Response", out var nestedResponse))
        {
            response = nestedResponse;
        }

        var score = response == null ? null : ReadLong(response.Value, "Score");
        var bestScore = response == null ? null : ReadLong(response.Value, "BestScore");
        int? rank = null;
        int? numEntries = null;
        if (response != null &&
            TryGetProperty(response.Value, "RankInfo", out var rankInfo) &&
            rankInfo.ValueKind == JsonValueKind.Object)
        {
            rank = ReadInt(rankInfo, "Rank");
            numEntries = ReadInt(rankInfo, "NumEntries");
        }

        var referenceEntries = new List<RetroAchievementsLeaderboardEntry>();
        if (response != null &&
            TryGetProperty(response.Value, "TopEntries", out var topEntries) &&
            topEntries.ValueKind == JsonValueKind.Array)
        {
            foreach (var item in topEntries.EnumerateArray())
            {
                var entryRank = ReadInt(item, "Rank");
                var entryUser = ReadString(item, "User") ?? string.Empty;
                var entryScore = ReadLong(item, "Score");
                if (entryRank is not > 0 || string.IsNullOrWhiteSpace(entryUser)) continue;
                referenceEntries.Add(new RetroAchievementsLeaderboardEntry
                {
                    Rank = entryRank.Value,
                    User = entryUser,
                    Score = entryScore,
                    FormattedScore = entryScore == null
                        ? string.Empty
                        : FormatLeaderboardSubmittedValue(leaderboardId, entryScore.Value.ToString(CultureInfo.InvariantCulture))
                });
            }
        }

        _session.LeaderboardsById.TryGetValue(leaderboardId ?? 0, out var leaderboard);
        var canonicalValue = score?.ToString(CultureInfo.InvariantCulture) ?? submittedValue;
        return new RecentLeaderboardSubmission
        {
            ObservedAtUtc = DateTime.UtcNow,
            Title = leaderboard?.Title ?? string.Empty,
            User = _session.Username,
            Value = canonicalValue,
            FormattedValue = leaderboardId is > 0
                ? FormatLeaderboardSubmittedValue(leaderboardId, canonicalValue)
                : submittedFormattedValue,
            Rank = rank,
            NumEntries = numEntries,
            BestScore = bestScore,
            BestFormattedScore = bestScore == null
                ? string.Empty
                : FormatLeaderboardSubmittedValue(leaderboardId, bestScore.Value.ToString(CultureInfo.InvariantCulture)),
            ReferenceEntries = referenceEntries
        };
    }

    private static bool IsLeaderboardResultState(string state)
    ...
    private static bool IsLoginAction(string action)
        => action.Equals("login", StringComparison.OrdinalIgnoreCase)
            || action.Equals("login2", StringComparison.OrdinalIgnoreCase);
    ...
    private async Task UpdateSessionAsync(Action<RetroAchievementsSessionSnapshot> update, CancellationToken cancellationToken)
    {
        ...
        await PublishAsync("retroachievements.session.updated", new
        {
            ...
            Session = new
            {
                ...
                _session.GameTitle,
                _session.GameHash,
                _session.GameImageIconUrl,
                _session.GameImageIconRemoteUrl,
                _session.Hardcore,
                LeaderboardsEnabled = AreLeaderboardsEnabled(),
                _session.Score,
                _session.SoftcoreScore,
                _session.RichPresence,
                _session.AchievementCount,
                _session.LeaderboardCount,
                _session.CatalogRevision,
                _session.SessionUnlockedAchievements,
                _session.LastEventAt
            }
        }, cancellationToken);
    }

    private Task PublishCatalogAsync(CancellationToken cancellationToken, string? correlationId = null)
        => PublishAsync("retroachievements.catalog.updated", new
        {
            Source = "retroachievements.catalog",
            Confidence = "high",
            _session.GameId,
            _session.GameTitle,
            Revision = _session.CatalogRevision,
            AchievementCount = _session.AchievementsById.Count,
            LeaderboardCount = _session.LeaderboardsById.Count,
            AchievementsById = _session.AchievementsById,
            LeaderboardsById = _session.LeaderboardsById
        }, cancellationToken, correlationId);

    private static string BuildCatalogRevision(
        IReadOnlyDictionary<int, RetroAchievementsAchievementInfo> achievements,
        IReadOnlyDictionary<int, RetroAchievementsLeaderboardInfo> leaderboards)
    {
        var value = string.Join('|', achievements.OrderBy(pair => pair.Key).Select(pair => $"A:{pair.Key}:{pair.Value.Title}")) +
                    "||" +
                    string.Join('|', leaderboards.OrderBy(pair => pair.Key).Select(pair =>
                        $"L:{pair.Key}:{pair.Value.Title}:{pair.Value.TopUser}:{pair.Value.TopFormattedScore}:{pair.Value.BadgeName}"));
    ...
    {
        var versionedPayload = JsonSerializer.SerializeToNode(payload, JsonOptions) as JsonObject ?? new JsonObject();
        versionedPayload["ContractVersion"] = 1;
        await _eventBus.PublishAsync(new EventEnvelope
        {
            ...
            Payload = JsonSerializer.SerializeToElement(versionedPayload, JsonOptions)
        });
    }
    ...
    private static int? TryReadLeaderboardId(IReadOnlyDictionary<string, string> parameters)
    {
        // Current rcheevos uses "i" for submitlbentry. Keep "l" as a legacy fallback.
        return TryReadInt(parameters, "i") ?? TryReadInt(parameters, "l");
    }
    ...
    private static long? ReadLong(JsonElement? root, string key)
    {
        if (root == null || !TryGetProperty(root.Value, key, out var value)) return null;
        return value.ValueKind switch
        {
    ...
    private static bool HasProperty(JsonElement element, string key)
        => element.ValueKind == JsonValueKind.Object && TryGetProperty(element, key, out _);
    ...
    private sealed class RecentLeaderboardSubmission
    {
        ...
        public string BestFormattedScore { get; init; } = string.Empty;
        public List<RetroAchievementsLeaderboardEntry> ReferenceEntries { get; init; } = new();
    }
...
public sealed class RetroAchievementsSessionSnapshot
{
    ...
    public Dictionary<int, RetroAchievementsLeaderboardInfo> LeaderboardsById { get; set; } = new();
    ...
    public RetroAchievementsSessionSnapshot Clone()
    {
        return new RetroAchievementsSessionSnapshot
        {
            ...
            LeaderboardsById = LeaderboardsById.ToDictionary(
                pair => pair.Key,
                pair => pair.Value.Clone()),
...
public sealed class RetroAchievementsLeaderboardInfo
{
    ...
    public string TopFormattedScore { get; set; } = string.Empty;
    public string BadgeName { get; set; } = string.Empty;
    public string BadgeUrl { get; set; } = string.Empty;
    public string BadgeRemoteUrl { get; set; } = string.Empty;

    public RetroAchievementsLeaderboardInfo Clone() => (RetroAchievementsLeaderboardInfo)MemberwiseClone();
}

public sealed class RetroAchievementsLeaderboardEntry
{
...
public sealed class RetroAchievementsChallengeEvent
{
    public string Source { get; set; } = "retroachievements.runtime";
...
public sealed class RetroAchievementsLeaderboardEvent
{
    public string Source { get; set; } = "retroachievements.runtime";
    public string State { get; set; } = "updated";
    public int? GameId { get; set; }
    ...
    public string ReferenceFormattedScore { get; set; } = string.Empty;
    public string BadgePath { get; set; } = string.Empty;
    public string BadgeRemoteUrl { get; set; } = string.Empty;
    public List<RetroAchievementsLeaderboardEntry> ReferenceEntries { get; set; } = new();
