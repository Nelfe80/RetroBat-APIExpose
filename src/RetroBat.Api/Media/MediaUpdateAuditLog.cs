...
internal static class MediaUpdateAuditLog
{
    ...
    {
        var path = Path.Combine(RetroBatPaths.PluginRoot, "logs", "media-update-audit.jsonl");
...
internal static class RefreshTrackingLog
{
    ...
    {
        var path = Path.Combine(RetroBatPaths.PluginRoot, "logs", "refresh-tracking.jsonl");
        var directory = Path.GetDirectoryName(path);
        if (!string.IsNullOrWhiteSpace(directory))
        {
            Directory.CreateDirectory(directory);
