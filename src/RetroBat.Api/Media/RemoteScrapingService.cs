...
public sealed class RemoteScrapingService
{
    ...
    {
        return Path.Combine(RetroBatPaths.PluginRoot, "logs", "remote-text-nochange-cache.json");
    }

    private static string ResolveRemoteExactLocalNoRetryCachePath()
    {
        return Path.Combine(RetroBatPaths.PluginRoot, "logs", "remote-exact-local-noretry-cache.json");
    }
